...
int main(int argc , char *argv[])
{
    ...
    printf("Numero de port pour la connexion au serveur : %d", ntohs(server.sin_port));
    ...
    while( (client_sock = accept(socket_desc, (struct sockaddr *)&client, (socklen_t*)&c)))
    {
        ...
        if (nb_connexions < NB_JOUEURS){
			...
			sprintf(messageJoueur,"Vous êtes le joueur : %d",nb_connexions+1);
        ...
        if(nb_connexions == NB_JOUEURS){
			...
			write(client_sock,partie_terminee, strlen(partie_terminee));	
			exit(0);
		}
    }
